...
create_vps() {
    ...
    $SUDO_CMD chown -R "$(id -u):$(id -g)" "$WORKDIR"
    ...
    wget -q --show-progress -O "$VM_IMG" "$IMG_URL"
    chmod 600 "$VM_IMG"

    # Dynamically package custom cloud-config if found locally
    CONFIG_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/cloud-config"
    if [ ! -d "$CONFIG_DIR" ] && [ -d "/home/zero/cloud-config" ]; then
        CONFIG_DIR="/home/zero/cloud-config"
    fi
...
EOF
    ...
    qemu-img resize "$VM_IMG" "+${DISK_ADD}G" > /dev/null 2>&1
